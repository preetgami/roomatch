<template>
  <div v-if="profile" class="flex flex-row items-center justify-between">
    <button
      ...
      hover::bg-gray-400  
      text-darkblue inline-block animate-slide-up-fade bg-gradient-to-br from-gray-900 to-gray-800 bg-clip-text p-2 text-4xl  sm:text-6xl md:text-7xl dark:from-gray-50 dark:to-gray-300 font-bold tracking-tighter text-transparent"
      @click="rejectUser"
    >
      <svg
    ...
    <MatchCard class="w-full" :profile="profile" />
    <button
      class="w-full flex items-center justify-center items-center pl-10 w-30
      text-darkblue inline-block animate-slide-up-fade bg-gradient-to-br from-gray-900 to-gray-800 bg-clip-text p-2 text-4xl  sm:text-6xl md:text-7xl dark:from-gray-50 dark:to-gray-300 font-bold tracking-tighter text-transparent"
      @click="acceptUser"
    >
      <svg
...
export default {
  ...
  methods:{
    ...
    acceptUser(){
      this.likeUser(this.profile.uid)
    },
    profiles(){
      if (this.getRecommendation){
      if(this.getRecommendation?.recommendations!==[]){
        this.profile= this.getRecommendation?.recommendations[0]
      }
    }
  },
    rejectUser(){
      this.rejectUsers(this.profile.uid)
    }
  }
};
</script>
